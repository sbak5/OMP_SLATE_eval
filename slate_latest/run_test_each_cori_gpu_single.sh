...
# reference (MKL_multithreaded)
      ...
      export MKL_NUM_THREADS=1
      srun -n $TOTAL_PROCS --ntasks-per-node $NSOCKETS -c $CORES_PER_TASK $HOST ./tester $app --origin h --target t --ref o --nb $NB_SCALAPACK --ib $IB --type $TYPE --lookahead 1 --panel-threads 1 --dim $SIZE --repeat $ITER $GRID &> ${EXP_DATE}/log_ref_${app}
      sleep 2
# reference (MPI_Only)
...
# SLATE with HCLIB OpenMP with gang scheduling
  ...
  do
      ...
      export OMP_NUM_THREADS=1
      export MKL_NUM_THREADS=1
      srun -n $TOTAL_PROCS --ntasks-per-node $NSOCKETS -c $CORES_PER_TASK $HOST ./tester $app --origin h --target t --ref o --nb 128 --type $TYPE --lookahead 1 --dim $SIZE --repeat $ITER $GRID --check n &> ${EXP_DATE}/log_ref_${app}
      sleep 2
