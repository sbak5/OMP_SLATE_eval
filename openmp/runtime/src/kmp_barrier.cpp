...
#if INTEROP_OMP
      int gang_sched = team->t.t_implicit_task_taskdata[0].td_icvs.gang_enabled;
      if (!gang_sched && bt != bs_forkjoin_barrier)
        interop_thread_scheduled(team->t.t_threads_desc, nproc);

      if (bt == bs_forkjoin_barrier) {
          interop_reserve_workers(nproc, team->t.t_parent->t.t_level);
          KA_TRACE(1,
            ("__kmp_linear_barrier_release: T#%d(%d:%d) reserve workers for %d OpenMP threads, %d\n"
             ,gtid, team->t.t_id, tid, nproc, team->t.t_nproc));
      }
...
#if 0 // CONV_OMP
        //if (i >= local_thr) {
          CthSetStrategyNodeQueue(team->t.t_threads_desc[i]);
          if (!node_lock_acquired) {
            CmiLock(CsvAccess(CsdNodeQueueLock));
            node_lock_acquired = 1;
          }
        //}
#endif    
...
//      }
      ...
      //CthAwakenBulk(team->t.t_threads_desc, local_thr, nproc);
#endif
    }
...
static int __kmp_barrier_template(enum barrier_type bt, int gtid, int is_split,
  ...
  if (!team->t.t_serialized) {
...
#if INTEROP_OMP 
      KMP_TEST_THEN_DEC32(&(this_thr->th.th_task_team->tt.tt_task_barrier_counts));
      KA_TRACE(
        2,
        ("__kmp_barrier: T#%d(%p:%d) decrement tt_task_barrier_counts: %d\n",
         gtid, this_thr->th.th_task_team, tid, this_thr->th.th_task_team->tt.tt_task_barrier_counts));
#endif
      status = 1;
#if USE_ITT_BUILD
      if (__itt_sync_create_ptr || KMP_ITT_DEBUG)
    ...
    if ((status == 1 || !is_split) && !cancelled) {
      ...
      if (KMP_MASTER_TID(tid)) {
        interop_master_wait(this_thr, &this_thr->th.th_team->t.t_num_barrier_counts, team->t.t_nproc-1, 0);
        KMP_MB();
...
void __kmp_join_barrier(int gtid) {
  ...
  if (KMP_MASTER_TID(tid)) {
    ...
    int num_local = 1;
    int num_tasks;
    interop_master_wait(this_thr, &team->t.t_num_barrier_counts, -1, 1);
    //__kmp_printf("__kmp_join_barrier: T#%d passed interop_master_wait\n");
...
*/
    interop_thread_scheduled(team->t.t_threads_desc, team->t.t_nproc);
    //__kmp_printf("__kmp_join_barrier: T#%d(%p:%d) passed interop_thread_scheduled\n");
#endif
    ...
    KA_TRACE(
        2,
        ("__kmp_join_barrier: T#%d(%p:%d) decrement tt_task_barrier_counts: %d\n",
         gtid, this_thr->th.th_task_team, tid, this_thr->th.th_task_team->tt.tt_task_barrier_counts));
/*
    kmp_bstate_t *thr_bar = &this_thr->th.th_bar[bs_forkjoin_barrier].bb;
    kmp_flag_64 flag(&thr_bar->b_go, KMP_BARRIER_STATE_BUMP);
    flag.wait(this_thr, TRUE USE_ITT_BUILD_ARG(itt_sync_obj));*/
    //__kmp_printf("__kmp_join_barrier: T#%d(%p:%d) decrement tt_task_barrier_counts: %d\n",
//         gtid, this_thr->th.th_task_team, tid, this_thr->th.th_task_team->tt.tt_task_barrier_counts);
    this_thr->th.th_bt = bs_forkjoin_barrier;
...
// To schedule gangs in this worker's gang_deq with earlier gang ids 
// After __kmp_internal_fork, this master thread may execute the parallel region with blocking barriers,
// which may cause deadlock on gang scheduling of ULTs 
