...
#if KMP_OS_UNIX 
#if INTEROP_OMP
#include "interop_layer/interop_thread.h"
#else
...
#endif
#if INTEROP_OMP
  void *interop_task; // Pointer of a structure to schedule this task in the interoperable runtime scheduler
#endif
  /*  private vars  */
} kmp_task_t;
...
#if INTEROP_OMP 
  /* In Charm++/OpenMP, worker user level threads may not pass barrier until the master user-level thread enters __kmp_task_team_wait 
   * This can happen because we don't have gather phase in __kmp_barrier to prevent worker threads from being suspended earlier while master thread creating tasks. This variable
   * ensures that worker threads waiting for the master thread to finish its task creation
   */
  KMP_ALIGN_CACHE 
    kmp_int32 tt_master_thread_wait;
  KMP_ALIGN_CACHE
    kmp_int32 tt_task_barrier_counts;
#endif
...
#endif
#if INTEROP_OMP 
  /* core id where this thread running(if it is master thread) */
  kmp_int32 th_cur_coreID;
  /* flag to represent whether this thread is replaced with a convthread */
  kmp_int32 th_worker_obtained;
  kmp_int32 th_ext_thread;
  enum barrier_type th_bt;
#endif
#if KMP_OS_UNIX
...
#if INTEROP_OMP 
  KMP_ALIGN_CACHE kmp_thread_t *t_threads_desc;
#endif
  kmp_taskdata_t
...
#if INTEROP_OMP
extern void __kmp_set_gang_sched(int gtid, int reset);
#endif
...
KMP_EXPORT void *__kmpc_threadprivate_cached(ident_t *loc, kmp_int32 global_tid,
                                             ...
                                             void ***cache);
#if INTEROP_OMP 
KMP_EXPORT void __kmp_wake_up_suspended_thread(int gtid);
#endif
