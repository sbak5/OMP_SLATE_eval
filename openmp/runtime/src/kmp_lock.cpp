...
int __kmp_acquire_nested_queuing_lock(kmp_queuing_lock_t *lck, kmp_int32 gtid) {
  KMP_DEBUG_ASSERT(gtid >= 0);
  KA_TRACE(1000,
           ("__kmp_acquire_nested_queuing_lock: lck:%p, T#%d entering\n", lck, gtid));
  std::atomic_thread_fence(std::memory_order_acquire);
  if (__kmp_get_queuing_lock_owner(lck) == gtid) {
    __atomic_fetch_add(&lck->lk.depth_locked, 1, __ATOMIC_RELEASE);
    KA_TRACE(1000, ("__kmp_acquire_nested_queuing_lock: lck:%p, T#%d exiting: tried to acquire the same lck, owner_id: %d, depth_locked: %d\n", lck, gtid, __kmp_get_queuing_lock_owner(lck), lck->lk.depth_locked));

    return KMP_LOCK_ACQUIRED_NEXT;
  } else {
    KA_TRACE(1000, ("__kmp_acquire_nested_queuing_lock: lck:%p, T#%d exiting: tried to acquire new lck, owner_id: %d, depth_locked: %d\n",
                      lck, gtid, __kmp_get_queuing_lock_owner(lck), lck->lk.depth_locked));
    ...
    KMP_MB();
//    lck->lk.depth_locked = 1;
    TCW_4(lck->lk.depth_locked, 1);
    KMP_MB();
    TCW_4(lck->lk.owner_id,gtid + 1);
    return KMP_LOCK_ACQUIRED_FIRST;
  }
}
...
int __kmp_release_nested_queuing_lock(kmp_queuing_lock_t *lck, kmp_int32 gtid) {
  ...
  KMP_MB();
  if (__atomic_sub_fetch(&lck->lk.depth_locked, 1, __ATOMIC_ACQ_REL) == 0) { //--(lck->lk.depth_locked) == 0) {
    KMP_MB();
    TCW_4(lck->lk.owner_id, 0);
...
void __kmp_destroy_nested_queuing_lock(kmp_queuing_lock_t *lck) {
    KA_TRACE(1000, ("__kmp_destroy_nested_queuing_lock: lck:%p, T#%d destroying lck, owner_id: %d, depth_locked: %d\n",
                      lck, __kmp_get_gtid(), __kmp_get_queuing_lock_owner(lck), lck->lk.depth_locked));
