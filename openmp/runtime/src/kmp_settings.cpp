...
#if INTEROP_OMP 
  int nth = interop_get_num_workers() * interop_get_num_workers() * 2;
#else
  int nth = 32;
...
// -----------------------------------------------------------------------------
// OMP_GANG_SCHED
static void __kmp_stg_parse_omp_gang_sched(char const *name, char const *value,
                                            void *data) {
  __kmp_stg_parse_bool(name, value, &(__kmp_gang_sched));
} // __kmp_stg_parse_gang_sched

static void __kmp_stg_print_omp_gang_sched(kmp_str_buf_t *buffer,
                                            char const *name, void *data) {
  __kmp_stg_print_bool(buffer, name, __kmp_gang_sched);
} // __kmp_stg_print_gang_sched
#endif
...
static kmp_setting_t __kmp_stg_table[] = {
    ...
    {"OMP_SCHEDULE", __kmp_stg_parse_omp_schedule, __kmp_stg_print_omp_schedule,
     NULL, 0, 0},
#if INTEROP_OMP
    {"OMP_GANG_SCHED", __kmp_stg_parse_omp_gang_sched, __kmp_stg_print_omp_gang_sched,
     NULL, 0, 0},
#endif
#if KMP_USE_HIER_SCHED
