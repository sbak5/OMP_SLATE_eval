...
#if KMP_DEBUG
  if (status) {
      KC_TRACE(1, ("__kmp_enter_single: T#%d will enter single construct\n ",
                    gtid));
...
#if INTEROP_OMP
  // this parallel region has num_threads clauses
  if (!this_thr->th.th_set_nproc) {
      new_nthreads = interop_get_dynamic_workers(set_nthreads); //parent_team->t.t_level);
  }
//  interop_reserve_workers(new_nthreads, parent_team->t.t_level);
#else
...
#if INTEROP_OMP 
      team->t.t_threads_desc[i] = thr->th.th_info.ds.ds_thread;
#endif
...
#if INTEROP_OMP 
  interop_suspend_worker(global_tid, this_thr->th.th_team);
#endif
...
/* TODO: revert workaround for Intel(R) 64 tracker #96 */
  ...
  { // KMP_TIME_BLOCK
    ...
    KA_TRACE(1, ("__kmp_fork_call: enter T#%d\n", gtid));
...
#if INTEROP_OMP 
    interop_suspend_worker(gtid, parent_team);
#endif
#if OMPT_SUPPORT
    ompt_data_t ompt_parallel_data = ompt_data_none;
...
// only one notification scheme (either "submit" or "forking/joined", not both)
    ...
    /* Invoke microtask for MASTER thread */
    KA_TRACE(3, ("__kmp_fork_call: T#%d(%d:0) invoke microtask = %p\n", gtid,
...
// works, only adjust nesting levels
   ...
   // if (team->t.t_level == 0)
    //if (KMP_MASTER_GTID(gtid))  
      //__kmp_wake_up_suspended_thread(gtid);
    interop_release_workers(); //team->t.t_nproc, parent_team->t.t_level);
...
#endif
#if INTEROP_OMP
  KA_TRACE(1, ("__kmp_join_call: T#%d, level:%d \n", gtid, parent_team->t.t_level));
  interop_release_workers(); //team->t.t_nproc, parent_team->t.t_level);
  interop_thread_local(inOpenMP)=0;
#endif

  KMP_MB();
  KA_TRACE(3, ("__kmp_join_call: exit T#%d\n", gtid));
}
...
#if INTEROP_OMP
void __kmp_set_gang_sched(int gtid, int reset) {
  kmp_info_t *thread  = __kmp_threads[gtid];
  if (reset) {
    thread->th.th_current_task->td_icvs.gang_enabled = 0;
  } else {
    thread->th.th_current_task->td_icvs.gang_enabled = 1;
  }
}
#endif
...
#if INTEROP_OMP
  team->t.t_threads_desc = 
      (kmp_thread_t *)__kmp_allocate(sizeof(kmp_thread_t) * max_nth);
#endif
...
#if INTEROP_OMP 
  __kmp_free(team->t.t_threads_desc);
#endif
...
#if INTEROP_OMP
  interop_root_initialize(root_thread, gtid, initial_thread);
#endif
...
#if INTEROP_OMP 
  new_thr->th.th_worker_obtained = 0;
  new_thr->th.th_ext_thread = 0;
#endif
...
#if INTEROP_OMP 
          team->t.t_threads_desc[f] = new_worker->th.th_info.ds.ds_thread;
#endif
          KA_TRACE(20,
...
#endif
#if INTEROP_OMP
  interop_set_global_variable(gtid, 1, 0);
#endif
    ...
    if (TCR_SYNC_PTR(*pteam) && !TCR_4(__kmp_global.g.g_done)) {
      ...
      if (TCR_SYNC_PTR((*pteam)->t.t_pkfn) != NULL) {
...
#endif
#if INTEROP_OMP && KMP_DEBUG
        KA_TRACE(20, ("[%llu] Beginning invoke task, thread: %p, PE: %d, gtid: %d, master_tid: %d, t_invoke: %p,t_num_shared_tasks:%d\n", __kmp_now_nsec(), this_thr, interop_get_worker_id(), gtid, __kmp_tid_from_gtid(gtid),(*pteam)->t.t_invoke ,(*pteam)->t.t_num_shared_tasks));
#endif
        ...
        if ((*pteam)->t.t_num_shared_tasks > (*pteam)->t.t_nproc-1)
          KA_TRACE(20,( "num_shared_tasks should not be less than number of threads in the team"));
#endif
        KMP_TEST_THEN_DEC32(&(*pteam)->t.t_num_shared_tasks);
        KA_TRACE(20, ("[%llu] thread: %p, PE: %d, gtid: %d, master_tid: %d, t_num_shared_tasks:%d\n", __kmp_now_nsec(), this_thr, interop_get_worker_id() , gtid, __kmp_tid_from_gtid(gtid), (*pteam)->t.t_num_shared_tasks));
#endif
        KMP_ASSERT(rc);
...
#pragma warning(pop)
  ...
  KA_TRACE(1, ("__kmp_internal_end_dest: T#%d\n", gtid));
...
#if (KMP_OS_UNIX && KMP_DYNAMIC_LIB) || INTEROP_OMP
...
void __kmp_internal_end_thread(int gtid_req) {
  ...
  int prevCoreID=0;;
//  if (gtid_req < 0)
  prevCoreID = __kmp_threads[gtid]->th.th_cur_coreID;
#endif
    ...
    if (gtid == KMP_GTID_SHUTDOWN) {
      ...
      if (__kmp_root[gtid]->r.r_active) {
        ...
        KA_TRACE(10, ("__kmp_internal_end_thread: unregistering sibling T#%d\n",
                      gtid));
#if INTEROP_OMP
        int prevCoreID = __kmp_threads[gtid]->th.th_cur_coreID;
        interop_thread_finalize(gtid, prevCoreID, gtid_req);
#endif
...
//#if KMP_DYNAMIC_LIB || INTEROP_OMP
#if 0 // INTEROP_OMP
  interop_thread_finalize(gtid, prevCoreID, gtid_req);
#endif
#if OMP_50_ENABLED
...
// Print all messages in message catalog for testing purposes.
  ...
  KMP_MB();
#if INTEROP_OMP
  interop_post_initialize();//argc, argv);
#endif
