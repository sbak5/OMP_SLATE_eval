...
#define INTEROP_OMP_TASK (1 & INTEROP_OMP)

#define INTEROP_TASKLOOP 0
/* forward declaration */
...
#if 0 // INTEROP_OMP
      KA_TRACE(3, ("__kmp_push_task: T#%d enabling task_team: %p, taskdata->td_task_team: %p\n", gtid, task_team, taskdata->td_task_team));
#endif
...
#if INTEROP_OMP_TASK
  interop_schedule_task(task->interop_task);
#else
...
#if INTEROP_OMP_TASK
  // Create a task for the ULT runtime to schedule OpenMP task in their workers
  task->interop_task = interop_create_task(interop_execute_task, task); 
#endif
...
#if INTEROP_OMP_TASK
void __kmp_invoke_task(kmp_int32 gtid, kmp_task_t *task,
                              kmp_taskdata_t *current_task) {
#else
static void __kmp_invoke_task(kmp_int32 gtid, kmp_task_t *task,
  ...
  kmp_info_t *thread;
#if INTEROP_OMP
  if (__kmp_threads[gtid]->th.th_task_team != taskdata->td_task_team) {
      KA_TRACE(
          30, ("__kmp_invoke_task(enter): T#%d invoking task from task_team %p, th_task_team=%p\n",
               gtid, taskdata->td_task_team, __kmp_threads[gtid]->th.th_task_team));
  }
#endif
  int discard = 0 /* false */;
  KA_TRACE(
...
static kmp_task_t *__kmp_remove_my_task(kmp_info_t *thread, kmp_int32 gtid,
  ...
  KA_TRACE(10, ("__kmp_remove_my_task(enter): T#%d ntasks=%d head=%u tail=%u\n",
                ...
                thread_data->td.td_deque_head, thread_data->td.td_deque_tail));
#if INTEROP_OMP_TASK
  task = (kmp_task_t*)interop_pop_task(gtid, is_constrained, thread->th.th_current_task);
#else
...
#if INTEROP_OMP_TASK
  task = (kmp_task_t*)interop_steal_task(gtid, is_constrained, __kmp_threads[gtid]->th.th_current_task);
  if (!task)
    return NULL;
...
#if INTEROP_OMP
    interop_execute_gang_task(!is_constrained); 
//    hclib_schedule_suspend(NULL, 0);
    ...
    if (final_spin &&
      ...
      if (!*thread_finished) {
        ...
        KA_TRACE(5, ("__kmp_execute_tasks_template: T#%d dec "
...
static void __kmp_enable_tasking(kmp_task_team_t *task_team,
  ...
  KA_TRACE(3, ("__kmp_enable_tasking(enter): T#%d\n",
...
#if INTEROP_OMP 
  TCW_4(task_team->tt.tt_master_thread_wait, FALSE);
  TCW_4(task_team->tt.tt_task_barrier_counts, task_team->tt.tt_nproc-1);
#endif
...
void __kmp_task_team_setup(kmp_info_t *this_thr, kmp_team_t *team, int always) {
  ...
  if (team->t.t_task_team[this_thr->th.th_task_state] == NULL &&
    ...
    KA_TRACE(5, ("__kmp_task_team_setup: Master T#%d created new task_team %p "
  ...
  if (team->t.t_nproc > 1) {
    ...
    if (team->t.t_task_team[other_team] == NULL) { // setup other team as well
      team->t.t_task_team[other_team] =
          __kmp_allocate_task_team(this_thr, team);
      KA_TRACE(5, ("__kmp_task_team_setup: Master T#%d created second new "
      ...
      KA_TRACE(5, ("__kmp_task_team_setup: Master T#%d reset next task_team "
...
#if INTEROP_OMP 
  /* guarantee all the worker threads reach the join barrier point - all the worker threads finish to execute their parallel regions and create tasks */
  void *msg;
  KA_TRACE(1, ("__kmp_task_team_wait: Master T#%d waiting for all tasks "
                    "(for unfinished_threads to reach 0) on task_team = %p, task_barrier_counts: %d\n",
                    __kmp_gtid_from_thread(this_thr), task_team, task_team->tt.tt_task_barrier_counts));

  /* In some cases, all the PEs can be busy 
   * so all the ULTs in the current PE's taskqueue 
   * may not be executed shortly.
   * To prevent this, master PE needs to execute the ULTs in this loop
   * */
  interop_master_wait(this_thr, &task_team->tt.tt_task_barrier_counts, task_team->tt.tt_nproc-1, 0);
#endif
...
#if INTEROP_OMP
  TCW_4(task_team->tt.tt_master_thread_wait, TRUE);
#endif
...
void __kmp_taskloop_linear(ident_t *loc, int gtid, kmp_task_t *task,
  ...
  KA_TRACE(20, ("__kmp_taskloop_linear: T#%d: %lld tasks, grainsize %lld, "
                ...
                task_dup));
#if INTEROP_TASKLOOP
  if (current_task->td_icvs.gang_enabled) {
      int num_request = num_tasks;
      if (num_tasks >= interop_get_num_workers())
          num_request = interop_get_num_workers();
      int num_workers = interop_get_dynamic_workers(num_request); 
//      num_tasks = num_workers;
      interop_reserve_workers(num_workers, thread->th.th_team->t.t_level);

      interop_set_worker_ntasks(num_tasks/num_workers, num_tasks % num_workers);
  }
#endif
...
//#endif // we use linear by default
  else {
...
#if INTEROP_TASKLOOP
  if (thread->th.th_current_task->gang_enabled)
      interop_release_workers(); // ,__kmp_threads[gtid]->th.th_team->t.t_level);
#endif
