...
#if INTEROP_OMP_TASK
  task = (kmp_task_t*)interop_pop_task();
#else
...
#if INTEROP_OMP_TASK
  task = (kmp_task_t*)interop_steal_task();
  if (!task)
    return NULL;
...
void __kmp_task_team_wait(
   ...
   * */
  interop_master_wait(this_thr, &task_team->tt.tt_task_barrier_counts, task_team->tt.tt_nproc-1, 1);
#endif
