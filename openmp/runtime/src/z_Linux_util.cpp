...
static void *__kmp_launch_worker(void *ptr) {
...
#if !INTEROP_OMP && (KMP_OS_LINUX || KMP_OS_DRAGONFLY || KMP_OS_FREEBSD || KMP_OS_NETBSD ||     \
        KMP_OS_OPENBSD)
...
#if INTEROP_OMP
  interop_create_thread(__kmp_launch_worker, th, stack_size);
...
#endif
  KF_TRACE(3, ("__kmp_suspend_template: T#%d set sleep bit for spin(%p)==%x,"
               " was %x\n",
               th_gtid, flag->get(), flag->load(), old_spin));

#if INTEROP_OMP 
  KMP_TEST_THEN_DEC32((kmp_int32 *) &(th->th.th_team->t.t_num_barrier_counts));
  KF_TRACE( 3, ( "__kmp_suspend_template: [%d] T#%d reduced barrier counts: %d, %p \n", interop_get_worker_id() ,
                     th_gtid, th->th.th_team->t.t_num_barrier_counts, &(th->th.th_team->t.t_num_barrier_counts)) );
    
  if (!th->th.th_team->t.t_implicit_task_taskdata[0].td_icvs.gang_enabled 
             || th->th.th_team->t.t_parent->t.t_level == 0 || th->th.th_bt == bs_forkjoin_barrier)
      interop_suspend_thread(th_gtid);
 // th->th.th_reap_state = KMP_NOT_SAFE_TO_REAP;
//  KF_TRACE( 2, ( "__kmp_suspend_template: [%d] T#%d flag_value: %d, flag_notdone_check(): %d\n", interop_get_worker_id (),
 //                    th_gtid, *flag->get(), flag->notdone_check()) );
    ...
    while (flag->is_sleeping()) {
      ...
      if (!deactivated) {
        ...
        if (th->th.th_active_in_pool) {
          ...
          KMP_DEBUG_ASSERT(__kmp_thread_pool_active_nth.load(std::memory_order_acquire) >= 0);
        }
        deactivated = TRUE;
      }
