...
#endif // INTEROP_OMP 
#if INTEROP_OMP
  int suspend_called = 0; 
  int suspend_only_once = this_thr->th.th_team->t.t_implicit_task_taskdata[0].td_icvs.gang_enabled && this_thr->th.th_team->t.t_parent->t.t_level > 0;
#endif
  // Main wait spin loop
...
#if INTEROP_OMP
    if ((task_team != NULL)) {
      if ( (!interop_master_running_status() && !TCR_4(task_team->tt.tt_master_thread_wait)) || TCR_4(task_team->tt.tt_found_tasks)) {
//      if ( (!TCR_4(task_team->tt.tt_master_thread_wait)) || TCR_4(task_team->tt.tt_found_tasks)) {
        continue;
      }
    }
#else
...
//    if(task_team)
    ...
    if (task_team) {
      ...
      KF_TRACE(5, ("__kmp_wait_sleep: T#%d suspend time reached, th_task_team: %p, task_team: %p, masterRunning: %d\n", th_gtid, this_thr->th.th_task_team, task_team, interop_master_running_status()));
    ...
    if (!KMP_MASTER_GTID(th_gtid)) {
        if (!(suspend_only_once && suspend_called)) {
#endif
...
#if KMP_DEBUG || INTEROP_OMP
  int gtid = TCR_4(__kmp_init_gtid) ? __kmp_get_gtid() : -1;
#endif
  KF_TRACE(3, ("__kmp_release: T#%d releasing flag(%x)\n", gtid, flag->get()));
    ...
    if (flag->is_any_sleeping()) {
      for (unsigned int i = 0; i < flag->get_num_waiters(); ++i) {
        ...
        if (waiter) {
          ...
          KF_TRACE(2, ("__kmp_release: T#%d waking up thread T#%d since sleep "
                        "flag(%p) set\n",
                        gtid, wait_gtid, flag->get()));
#if INTEROP_OMP
          flag->unset_sleeping();
          KMP_MB();
          if (!this_thr->th.th_team->t.t_implicit_task_taskdata[0].td_icvs.gang_enabled || this_thr->th.th_team->t.t_parent->t.t_level == 0 || this_thr->th.th_bt == bs_forkjoin_barrier) 
          interop_resume_thread(waiter);
#else
