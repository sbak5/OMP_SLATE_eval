...
//#pragma omp single 
//                            {
                                // Wavefront computation within this tile
                                // The master thread computes the first few diagonals until the length of diagonal reaches the number of threads -> Triangle
                                // And then each diagonal within the hexagon in the middle of the matrix will be executed in parallel
//                                int num_threads = omp_get_num_threads();
                                // Serial execution of the 1st triangle 
                                //
                                int cur_width = 1;
/*                                for (ii = 1; ii < num_serial_rows; ++ii) {
                                    for ( jj = 1; jj < cur_width; ++jj ) {
                                        const signed char char_from_1 = string_1[(j-1)*tile_width+(jj-1)];
                                        const signed char char_from_2 = string_2[(i-1)*tile_height+(ii-1)];

                                        const int diag_score = curr_tile[ii-1][jj-1] + alignment_score_matrix[char_from_2][char_from_1];
                                        const int left_score = curr_tile[ii  ][jj-1] + alignment_score_matrix[char_from_1][GAP];
                                        const int  top_score = curr_tile[ii-1][jj  ] + alignment_score_matrix[GAP][char_from_2];

                                        const int bigger_of_left_top = (left_score > top_score) ? left_score : top_score;
                                        curr_tile[ii][jj] = (bigger_of_left_top > diag_score) ? bigger_of_left_top : diag_score;
                                    }
                                    cur_width--;
                                }*/
                                // Wavefront execution of the middle hexagon
//                                cur_width = ;
#pragma omp parallel 
                                {
#pragma omp master 
                                    {
                                        ...
                                        for (ii=1; ii < tile_height+1; ++ii) {
                                                const int cur_x_base = ii;
                                                const int cur_y_base = 1;

                                                #pragma omp taskloop num_tasks(num_threads) // if (cur_width >= num_threads)
                                                for (int k =0; k < cur_width; k++) {
                                                    const int cur_x = cur_x_base -k;
                                                    const int cur_y = cur_y_base +k;
                                                    const signed char char_from_1 = string_1[(j-1)*tile_width+(cur_y-1)];
                                                    const signed char char_from_2 = string_2[(i-1)*tile_height+(cur_x-1)];

                                                    const int diag_score = curr_tile[cur_x-1][cur_y-1] + alignment_score_matrix[char_from_2][char_from_1];
                                                    const int left_score = curr_tile[cur_x  ][cur_y-1] + alignment_score_matrix[char_from_1][GAP];
                                                    const int  top_score = curr_tile[cur_x-1][cur_y  ] + alignment_score_matrix[GAP][char_from_2];

                                                    const int bigger_of_left_top = (left_score > top_score) ? left_score : top_score;
                                                    curr_tile[cur_x][cur_y] = (bigger_of_left_top > diag_score) ? bigger_of_left_top : diag_score;
                                                }
                                                cur_width++;
                                        }
                                        cur_width-=2;
                                        for (jj=2; jj < tile_width+1; ++jj) {
                                                const int cur_x_base = tile_height;
                                                const int cur_y_base = jj;
...
//#pragma omp for nowait
                        ...
                        free(curr_tile);
                        free(curr_tile_tmp);
                    }, *futures); // tile_matrix[i][j-1].right_column->get_future(),
