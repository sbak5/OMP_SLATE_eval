...
#include <errno.h>
//#define VERBOSE
//#define INTEROP_OMP_DEBUG 1
#if INTEROP_OMP
extern hclib_victim_t global_victim_policy;
#endif
...
void init_hclib_deque_t(hclib_deque_t *hcdeq, hclib_locale_t *locale) {
#if INTEROP_OMP
    hcdeq->deque_nb.head = hcdeq->deque_nb.tail = 0;
    hcdeq->deque_gen.head = hcdeq->deque_gen.tail = 0;
#else
    hcdeq->deque.head = hcdeq->deque.tail = 0;
#endif
//    hcdeq->deque_gang.head = hcdeq->deque_gang.tail = 0;

    hcdeq->locale = locale;
    hcdeq->ws = NULL;
    hcdeq->nnext = NULL;
#if INTEROP_OMP
    // First pointer points to this deque
    hcdeq->nlevel = (hclib_deque_t **)malloc(sizeof(hclib_deque_t*) * INIT_NEST_LEVEL);
    hcdeq->nlevel[0] = hcdeq;
    for (int i=1; i< INIT_NEST_LEVEL; i++)
        hcdeq->nlevel[i] = NULL;
#endif
    hcdeq->prev = NULL;
#ifdef BUCKET_DEQUE
...
#if INTEROP_OMP
    locale->gang_deq = (hclib_internal_deque_t *)malloc(sizeof(hclib_internal_deque_t));
#endif
...
#if INTEROP_OMP
    locale->gang_deq->head = locale->gang_deq->tail = 0;
    //init_hclib_deque_t(locale->gang_deq, locale);
#endif
...
static inline hclib_deque_t *get_deque_locale(hclib_worker_state *ws,
    ...
    assert(locale);
#if INTEROP_OMP
    hclib_deque_t *cur_deq = locale->deques[ws->id].nlevel[ws->internal_nest_level];
    if (!cur_deq) {
        hclib_deque_t *new_deq = (hclib_deque_t*)malloc(sizeof(hclib_deque_t));
        new_deq->deque_nb.head = new_deq->deque_nb.tail = 0;
        new_deq->deque_gen.head = new_deq->deque_gen.tail = 0;
        locale->deques[ws->id].nlevel[ws->internal_nest_level] = new_deq;
        cur_deq = new_deq;
...
int deque_push_locale(hclib_worker_state *ws, hclib_locale_t *locale,
    ...
    hclib_task_t *task = (hclib_task_t*)ele;
    hclib_deque_t *deq = NULL;
#if INTEROP_OMP
    if (ws->gang_monotonic_id[ws->internal_nest_level] !=-1 && !task->non_blocking) { // task->persistent) {
      task->gang_id = ws->gang_monotonic_id[ws->internal_nest_level];
      task->gang_nested_level = ws->gang_nested_level[ws->internal_nest_level];
      task->assigned_workers_ptr = &(ws->assigned_workers[ws->internal_nest_level]);
      task->assigned_worker_id = ws->cur_assigned_idx;

//      pthread_mutex_lock(&ws->per_worker_mutex);
      int return_value = deque_push(locale->gang_deq, ele);
//      pthread_mutex_unlock(&ws->per_worker_mutex);
      return return_value;
    }
    else { 
      deq = get_deque_locale(ws,locale);
      HASSERT(deq);
      if (task->non_blocking)
        return deque_push(&deq->deque_nb, ele);
      else
        return deque_push(&deq->deque_gen, ele);
    }
#else
    deq = get_deque_locale(ws,locale);
...
#if INTEROP_OMP
        hclib_internal_deque_t *deq_gen = &(locale->deques[wid].deque_gen);
        hclib_internal_deque_t *deq_nb = &(locale->deques[wid].deque_nb);
        const int tail_gen = deq_gen->tail;
        const int head_gen = deq_gen->head;
        const int tail_nb = deq_nb->tail;
        const int head_nb = deq_nb->head;

        sum_work += (tail_gen - head_gen) + (tail_nb - head_nb);
#else
...
#if INTEROP_OMP
        count += deque_size(&(deqs[i].deque_nb));
        count += deque_size(&(deqs[i].deque_gen));
#else
...
static inline hclib_task_t* deque_pop_queues(hclib_deque_t *deqs, int nest_level, int nb_only) {
    int nstolen=0;
    hclib_deque_t *cur_deq_ptr = deqs->nlevel[nest_level];
    if (cur_deq_ptr) {
        if (!nb_only)
            return deque_pop(&(cur_deq_ptr->deque_gen));
        if (!nstolen) {
            return deque_pop(&(cur_deq_ptr->deque_nb));
    //        if (!nstolen) 
    //            nstolen = deque_steal_gang(&(deqs[victim_id].deque_gang), stolen);        
        }
    }
    return NULL;
}
 ...
 */
hclib_task_t *locale_pop_task(hclib_worker_state *ws, int non_blocking) {
    int i;
    const int wid = ws->id;
    hclib_worker_paths *paths = ws->paths;
    hclib_locality_path *pop = paths->pop_path;
...
#if INTEROP_OMP
        task = deque_pop_queues(&(locale->deques[wid]), ws->internal_nest_level, non_blocking);
//        if (!task)
//          task = deque_pop(&(locale->deques[wid].deque_nb));
#else
        task = deque_pop(&(locale->deques[wid].deque));
#endif
        if (task) {
#ifdef VERBOSE
        fprintf(stderr, "locale_pop_task: wid=%d i=%d locale=%p "
...
static inline int deque_steal_queues(hclib_deque_t *deqs, int target_nest_level, void **stolen, int nb_only) {
    int nstolen=0;
    if (target_nest_level >=0) { // cur_gang_id == victim_gang_id
        hclib_deque_t *cur_deq_ptr = deqs->nlevel[target_nest_level];
        if (cur_deq_ptr) {
            if (!nb_only)
                nstolen = deque_steal(&(cur_deq_ptr->deque_gen), stolen);
            if (!nstolen) {
                nstolen = deque_steal(&(cur_deq_ptr->deque_nb), stolen);
        //        if (!nstolen) 
        //            nstolen = deque_steal_gang(&(deqs[victim_id].deque_gang), stolen);        
            }
        }
...
static int find_victim_worker_id(hclib_worker_state *ws, int nest_level) {
    int nworkers = ws->assigned_workers[nest_level].num_workers; 
    int prev_history_idx = ws->assigned_workers[nest_level].cur_prev_idx;
    int victim_idx = ws->assigned_workers[nest_level].prev_victim_idx[prev_history_idx];
    if (victim_idx < 0) {
        victim_idx = interop_get_random() % nworkers;
        HASSERT(ws->assigned_workers[nest_level].workers!=NULL);
        if (ws->assigned_workers[nest_level].workers[victim_idx].wid == ws->id)
            victim_idx=(victim_idx+1) % nworkers;
    }
    return victim_idx;
}

int locale_steal_gang(hclib_worker_state *ws, void **stolen, int *out_victim) {
    hclib_worker_paths *paths = ws->paths;
    hclib_locality_path *steal = paths->steal_path;
    // Try to steal its own taskqueue
    int nstolen = deque_steal_gang(steal->locales[0]->gang_deq, stolen);
    // We assume locales[0] is thread local locale (only accessible to each worker)
    if (!nstolen) {
        int victim_idx = interop_get_random() % (ws->nworkers-1);
        if (victim_idx >= ws->id)
            victim_idx++;
//        hclib_assigned_worker_info *victim  = &(ws->assigned_workers[ws->internal_nest_level].workers[victim_idx]); 
        steal = hc_context->workers[victim_idx]->paths->steal_path;
        nstolen = deque_steal_gang(steal->locales[0]->gang_deq, stolen);
    }
    return nstolen;
}
 ...
 */
int locale_steal_task(hclib_worker_state *ws, void **stolen, int *out_victim, int non_blocking) {
    int i, j;
    const int wid = ws->id;
    ...
    int nstolen = 0;
    if (!non_blocking) {
        nstolen = locale_steal_gang(ws, stolen, out_victim);
        if (nstolen)
            return nstolen;
    }
#endif
    ...
    for (i = 0; i < steal_path_length; i++) {
        ...
        hclib_deque_t *deqs = locale->deques;
#if INTEROP_OMP
        int cur_nest_level = ws->internal_nest_level;
        for (; cur_nest_level > 0; cur_nest_level--) {
            hclib_assigned_worker *cur_worker_list = &ws->assigned_workers[cur_nest_level]; 
            if (cur_worker_list->workers) {
                HASSERT(cur_nest_level > 0);
                int nstolen = 0;
                int victim_idx = find_victim_worker_id(ws, cur_nest_level);
                hclib_assigned_worker_info *victim  = &(cur_worker_list->workers[victim_idx]); 
                nstolen = deque_steal_queues(&deqs[victim->wid], victim->scheduled_internal_nest,  stolen, non_blocking);
                if (nstolen) {
                    //if (cur_nest_level != ws->internal_nest_level)
//                        fprintf(stderr, "locale_steal_task: ws=%p wid=%d cur_nest_level=%d internal_nest_level=%d\n", ws,
//                                wid, cur_nest_level, ws->internal_nest_level);

                    paths->last_successful_steal_locale = locale_index;
                    cur_worker_list->prev_victim_idx[cur_worker_list->cur_prev_idx]  = victim_idx;
                    
                    if (global_victim_policy == HCLIB_HYBRID_VICTIM)
                      cur_worker_list->cur_prev_idx = (cur_worker_list->cur_prev_idx+1)%SHUFFLE_LENGTH;
                    
                    *out_victim = victim->wid;
                    return nstolen;
                }
                cur_worker_list->prev_victim_idx[cur_worker_list->cur_prev_idx] = -1;
                
                if (global_victim_policy == HCLIB_HYBRID_VICTIM)
                  cur_worker_list->cur_prev_idx = (cur_worker_list->cur_prev_idx + SHUFFLE_LENGTH -1) % SHUFFLE_LENGTH;
            } else { // The following work-stealing is used for the first-level 
                if (paths->last_successful_wid >= 0) {
                    const int victim = paths->last_successful_wid;
                    int nstolen = 0;
                    nstolen = deque_steal_queues(&deqs[victim], 0, stolen, non_blocking);

                    if (nstolen) {
                        paths->last_successful_steal_locale = locale_index;
                        paths->last_successful_wid = victim;
                        *out_victim = victim;
        //                pthread_mutex_unlock(&ws->per_worker_mutex);
                        return nstolen;
                    }                                                                            
                }
                paths->last_successful_wid = -1;    
#endif
                for (j = ws->base_intra_socket_workers; j < ws->limit_intra_socket_workers; j++) {
                    const int victim = j;

#if INTEROP_OMP
                   int nstolen = deque_steal_queues(&deqs[victim], 0, stolen, non_blocking); 
#else
     /* deque_steal(&(deqs[victim].deque_gen), stolen); 
                    if (!nstolen)
                      nstolen = deque_steal(&(deqs[victim].deque_nb), stolen); */
...
#if INTEROP_OMP
                    int nstolen = deque_steal_queues(&deqs[victim], 0, stolen, non_blocking);  /* deque_steal(&(deqs[victim].deque_gen), stolen); 
                    if (!nstolen)
                      nstolen = deque_steal(&(deqs[victim].deque_nb), stolen); 
        */
#else
    ...
    // We finished our idle_status -> Wait for any producer
#if STEAL_OPT
    ws->idle_status = 1;
    atomic_fetch_add_explicit(&(hc_context->idle_cnt), 1, memory_order_relaxed);
#endif
//    pthread_mutex_unlock(&ws->per_worker_mutex);
    return 0;
}
