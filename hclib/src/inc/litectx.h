...
#if INTEROP_OMP
    struct LiteCtxStruct *volatile next;
    int scheduled; // Required to implement non-blocking barrier in OpenMP
    void *volatile task; // Used to schedule this ctx in hclib
#endif
    void *volatile arg1;
    void *volatile arg2;
    fcontext_t _fctx;
...
static __inline__ LiteCtx *LiteCtx_create(void (*fn)(LiteCtx*), int stack_size) {
    int ctx_stack_size = stack_size; 
    if (ctx_stack_size <=0)
      ctx_stack_size = LITECTX_SIZE;

    LiteCtx *ctx = (LiteCtx *)LITECTX_ALLOC(ctx_stack_size); //ctx_stack_size + sizeof(LiteCtx));
    if (!ctx) {
        fprintf(stderr, "Failed allocating litectx\n");
        exit(1);
    }
    char *const stack_top = ctx->_stack + (ctx_stack_size - sizeof(LiteCtx)); //ctx_stack_size;
    ctx->prev = NULL;
    ctx->arg1 = NULL;
    ctx->arg2 = NULL;
#if INTEROP_OMP
    ...
    ctx->task = NULL;
#endif
    ctx->_fctx = make_fcontext(stack_top, ctx_stack_size,
            (void (*)(void *))fn);
