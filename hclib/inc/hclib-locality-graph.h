...
typedef struct _hclib_locale_t {
    ...
    int reachable;
#if INTEROP_OMP
    // This queue is shared by tasks to be scheduled in gang scheduling
    // Each worker doesn't push tasks into this queue. 
    // Workers where master thread of each gang running pushes ULTs to this queue with global shared lock
    //  
    struct hclib_internal_deque_t *gang_deq;
#endif
...
extern struct hclib_task_t *locale_pop_task(hclib_worker_state *ws, int non_blocking);

extern int locale_steal_task(hclib_worker_state *ws, void **stolen,
        int *out_victim, int non_blocking);
